...
if request("action")="update" then
	...
	sql="select * from [information] where id="&request("id")&""	'根据取得的仪器 ID 进行修改
...
<form name="myform" action="uppro.asp" method="post" onSubmit="return addpro();">  <tr> 
    <td align="center"><font color="#FFFFFF">修改仪器</font></td>
  </tr>
  <tr> 
    <td valign="top" bgcolor="#FFFFFF"><br> 
      <table width="90%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="799AE1">
<%
sql="select * from [information] where id="&request("id")&";"	'查询对应的仪器信息
set rs=Server.CreateObject("ADODB.Recordset")
rs.open sql,conn,3,3
%>
...
<%
sql1="select * from [bigclass] where id="&rs("bigclassid")&";"	'查询该仪器对应的大类
...
%>
<%
sql2="select * from [bigclass] where id<>"&rs("bigclassid")&" order by paixu;"	'查询除了对应该仪器的所有大类
...
<select name="classid">
<%
sql1="select * from [class] where id="&rs("classid")&";"	'查询该仪器对应的小类
set rs1=Server.CreateObject("ADODB.Recordset")
rs1.open sql1,conn,3,3
%>
<option value="<%=rs1("id")%>"><%=rs1("mingcheng")%></option>
...
%>
<%
sql2="select * from [class] where id<>"&rs("classid")&" order by paixu;"	'查询除了对应该仪器的所有小类，不重复输出
set rs2=Server.CreateObject("ADODB.Recordset")
rs2.open sql2,conn,3,3
do while not rs2.eof
%>
...
&nbsp;&nbsp;仪器数量：<input name="shuliang" type="text" size="6" value="<%=rs("shuliang")%>">
