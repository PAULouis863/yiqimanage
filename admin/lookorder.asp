...
if request("order")<>"" and request("action")="update" then	'判断是否修改
	...
	if not rs.eof then
		...
		'当 right(request("zhuangtai"),1) 的值大与或等于 2 （已经发货了）并且数据库中的值小于 2 (商品数量没有被修改过)的时候才对商品的数量进行修改
			...
			for i=0 to ubound(information)	'循环输出商品 ID，有多少商品就对应商品 ID进行数量修改
...
<%
		for i=0 to ubound(information)	'此时变量 information 为数组形式
			sql2="select * from [information] where id="&trim(information(i))&""	'得到每个商品的 ID
			set rs2=Server.CreateObject("ADODB.Recordset")
			rs2.open sql2,conn,3,3
%>
...
<%
		for i=0 to ubound(shuliang)
			...
			set rs2=Server.CreateObject("ADODB.Recordset")
			rs2.open sql2,conn,3,3
			response.Write(rs2("shichang")) '输出商品会员价格
			response.Write("<br>")
			rs2.close
			set rs2=nothing
		next
...
<%
		for i=0 to ubound(shuliang)
			...
			set rs2=Server.CreateObject("ADODB.Recordset")
			rs2.open sql2,conn,3,3
			response.Write(rs2("shichang")) '输出商品成交价格(成交价格其实就是会员价格)
			response.Write("<br>")
			rs2.close
			set rs2=nothing
		next
...
<table width="600"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td><div align="center" class="style1">注：影片确定发货后，该影片数量将自动从库存中相应减少！</div></td>
  </tr>
</table>
